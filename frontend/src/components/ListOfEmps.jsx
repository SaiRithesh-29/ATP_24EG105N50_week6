...
function ListOfEmps() {
  ...
  const deleteEmpByID = async(id)=>{
    let res = await axios.delete(`https://employee-backend-api-y9a3.onrender.com/employee-api/employee/${id}`);
    if(res.status===200){
  ...
  async function getEmps() {
      let res = await fetch("https://employee-backend-api-y9a3.onrender.com/employee-api/employees");
      if (res.status === 200) {
